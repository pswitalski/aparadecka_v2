...
- Commit messages must follow Conventional Commits with a colon: `type(scope): description` (e.g. `feat(web): rebuild gallery`, `ci: split web deploy`). `type` is required; `scope` is optional. Enforced by the commit-msg hook via commitlint. Proposed commit messages must conform to this format. Do not add any annotations to the commit message stating that it was done in collaboration with an AI agent.
